...
설치할 게 없다 — 무료 API 키를 발급받아 `export ASTA_API_KEY=<키>`(설정이 `${ASTA_API_KEY}`를
확장)하면 되고, 사용은 Ai2의 약관 적용을 받는다([출처](#출처-attribution) 참고).
그런 다음 `setup/mcp.servers.resolved.json`의 `mcpServers` 항목을 `~/.claude.json`에 병합하고
Claude Code를 재시작하면 된다.
